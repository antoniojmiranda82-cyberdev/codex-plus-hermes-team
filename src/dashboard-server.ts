...
import { assertAssetDreamProfile, assetDreamRoster } from "./project-rosters.js";
import { OpenAICompatibleExecutor } from "./providers/openai-compatible.js";

const roster: DashboardAgent[] = assetDreamRoster.map(({ profile, displayName, role }) => ({ profile, displayName, role }));
...
const storePath = process.env.OPERATOR_TASK_STORE ?? ".operator/asset-dream/tasks.json";
...
async function route(req: IncomingMessage, res: ServerResponse) {
  ...
  if (req.method === "GET" && url.pathname === "/api/snapshot") {
    json(res, 200, {
      ...
      snapshot: buildDashboardSnapshot(operator, roster),
      health: getProviderHealth(),
      tasks: operator.listTasks()
    });
    return;
  }
  ...
  if (req.method === "POST" && url.pathname === "/api/tasks") {
    ...
    const task = operator.createTask({
      ...
      agentProfile: assertAssetDreamProfile(asString(body.agentProfile, "agentProfile")),
...
<style>
:root{color-scheme:dark;font-family:Inter,ui-sans-serif,system-ui;background:#05070b;color:#eef4ff}body{margin:0;background:radial-gradient(circle at 15% 0,#14213a 0,transparent 28%),#05070b}.wrap{max-width:1200px;margin:auto;padding:28px}.top{display:flex;gap:16px;align-items:end;justify-content:space-between;flex-wrap:wrap}.eyebrow{color:#7dd3fc;font-size:12px;letter-spacing:.18em;text-transform:uppercase}.title{font-size:clamp(28px,5vw,56px);font-weight:800;margin:6px 0}.muted{color:#8da1ba}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin-top:22px}.card{background:#0b111b;border:1px solid #1d2a3b;border-radius:18px;padding:16px;box-shadow:0 20px 55px #0007}.metric{font-size:32px;font-weight:800}.agents{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:12px}.agent{display:flex;justify-content:space-between;gap:12px}.pill{padding:5px 9px;border-radius:999px;background:#111c2b;font-size:12px}.busy{color:#fde68a}.idle,.online{color:#86efac}.blocked,.error,.offline{color:#fca5a5}.section{margin-top:28px}.task{display:grid;grid-template-columns:minmax(160px,1fr) 100px 110px auto;gap:10px;align-items:center;padding:12px 0;border-bottom:1px solid #182234}.task button,button,select,input,textarea{background:#111827;color:#eef4ff;border:1px solid #2a3b52;border-radius:10px;padding:9px}.task button{cursor:pointer}.form{display:grid;grid-template-columns:1fr 1fr;gap:10px}.form textarea{grid-column:1/-1;min-height:84px}.form .full{grid-column:1/-1}.toolbar{display:flex;gap:8px;flex-wrap:wrap}@media(max-width:700px){.task{grid-template-columns:1fr}.form{grid-template-columns:1fr}.form textarea,.form .full{grid-column:auto}}</style></head>
<body><main class="wrap"><div class="top"><div><div class="eyebrow">Quantum Shadow Operator · asset-dream</div><h1 class="title">Asset + Dream Command Center</h1><div class="muted">Isolated workforce for Asset Ave and Dream Blvd only.</div></div><button onclick="refresh()">Refresh</button></div>
<section id="metrics" class="grid"></section><section class="section"><h2>Routing + integrations</h2><div id="health" class="grid"></div></section><section class="section"><h2>Agent workforce</h2><div id="agents" class="agents"></div></section>
...
async function refresh(){const data=await call('/api/snapshot');const s=data.snapshot;const h=data.health;document.getElementById('metrics').innerHTML=[['Total tasks',s.totalTasks],['Asset Ave',s.businesses['asset-ave'].total],['Dream Blvd',s.businesses['dream-blvd'].total],['Blocked',s.businesses['asset-ave'].blocked+s.businesses['dream-blvd'].blocked]].map(x=>'<div class="card"><div class="muted">'+x[0]+'</div><div class="metric">'+x[1]+'</div></div>').join('');const routes=Object.entries(h.routes);document.getElementById('health').innerHTML=[['Gateway',h.gatewayConfigured],['Slack',h.slackConfigured],...routes].map(x=>'<div class="card"><strong>'+esc(x[0])+'</strong><div class="pill '+(x[1]?'online':'offline')+'">'+(x[1]?'configured':'not configured')+'</div></div>').join('');document.getElementById('agents').innerHTML=s.agents.map(a=>'<div class="card agent"><div><strong>'+esc(a.displayName)+'</strong><div class="muted">'+esc(a.profile)+'</div><div class="muted">Q '+a.queued+' · Run '+a.running+' · Done '+a.completed+'</div></div><span class="pill '+a.status+'">'+a.status+'</span></div>').join('');const f=document.getElementById('businessFilter').value;const tasks=data.tasks.filter(t=>f==='all'||t.business===f);document.getElementById('tasks').innerHTML=tasks.length?tasks.map(t=>'<div class="task"><div><strong>'+esc(t.title)+'</strong><div class="muted">'+esc(t.agentProfile)+' · '+esc(t.business)+'</div></div><span class="pill">'+esc(t.status)+'</span><span>Attempts '+t.attempts+'</span><div class="toolbar">'+(!t.approved&&t.approvalRequirement==='external_side_effect'?'<button onclick="act(\''+t.id+'\',\'approve\')">Approve</button>':'')+(t.status==='queued'?'<button onclick="act(\''+t.id+'\',\'run\')">Run</button>':'')+(t.status==='failed'?'<button onclick="act(\''+t.id+'\',\'retry\')">Retry</button>':'')+'</div></div>').join(''):'<div class="muted">No tasks yet.</div>'}
